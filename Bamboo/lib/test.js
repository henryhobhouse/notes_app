(function() {
  ...
  Tests = {
    ...
    run: function(testName, obj) {
      ...
      try {
        ...
        if (e.name === 'expectionError') {
          result.message = e.toString();
          logger.fail('expection failed in: ' + testName);
          showException(e);
          this.failed += 1;
        } else {
    ...
    runAll: function(tests) {
      if (typeof TuringTest !== 'undefined' && BambooLeaf.isLoading) {
  ...
  if (typeof window === 'undefined') {
    ...
    BambooLeaf.testRunner = { run: Tests.runAll };
  }
})();
